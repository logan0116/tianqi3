...
def train():
    ...
    node_inf = networkdeal.get_index()
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(loader)) as bar:
            for sp, r, tp in loader:
                ...
                random_entities = np.random.choice(node_size, size=r.size(), replace=False)
                sn = np.where(head_or_tail == 1, random_entities, sp)
                tn = np.where(head_or_tail == 0, random_entities, tp)

                r = r.to(device)
                sp, tp = torch.Tensor(node_inf[sp, :]).to(device), torch.Tensor(node_inf[tp, :]).to(device)
                sn, tn = torch.Tensor(node_inf[sn, :]).to(device), torch.Tensor(node_inf[tn, :]).to(device)
